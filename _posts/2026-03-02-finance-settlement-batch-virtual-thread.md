...
기존 Finance 부하 테스트에서 확인하지 못했던 병목이 있었다. 이전 테스트에서는 정산 데이터가 소규모였고 VU도 낮았기 때문에 드러나지 않았다. 이번에는 정산 데이터를 500건으로 늘리고 VU를 1,000까지 올려 전체 파이프라인을 다시 검증했다.

---

## 초기 확인 — Semaphore 50 제거

첫 번째로 살펴본 것은 `PaymentService`에 있는 `Semaphore(50)` 제한이었다.
...
주석에 근거가 있어 보이지만 실제 코드 구조를 분석하면 불필요하다.
...
정산 p95 19.5초는 단일 원인이 아니었다. 배치 all-or-nothing 실패, 순차 fallback, Kafka 불필요 재시도가 겹쳐서 만들어진 수치였다.

fallback에 Virtual Thread를 쓴 것은 복잡한 선택이 아니었다. 순차 처리를 병렬로 바꾸되, I/O 바운드 작업이기 때문에 새 스레드풀을 만들지 않고 JVM이 알아서 관리하게 했다. Java 21이라 가능했다.
...
> **p95가 평균보다 훨씬 높을 때는 꼬리 경로(fallback, 재시도)를 먼저 의심해야 한다.**
> 평균이 2초인데 p95가 19초라는 건, 대부분의 요청은 빠르지만 특정 경로에서 급격히 느려진다는 뜻이다. 이번 경우 그 경로는 배치 실패 후 순차 fallback이었다.
