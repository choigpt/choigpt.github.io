...
k6로 채팅 API에 부하 테스트를 실행한 결과, 메시지 전송 성공률이 0.9%였고, WebSocket 메시지 왕복 시간은 측정 자체가 불가능했다. 에러 로그를 분석한 결과 복수의 문제가 확인됐다. 테스트 데이터 설계 결함, 쿼리 구조 문제, 미등록된 Redis 리스너, N+1 쿼리가 동시에 존재했다.
...
k6의 STOMP 테스트에서 메시지를 보내도 WebSocket 클라이언트가 응답을 받지 못했다. 원인은 다음과 같다.
...
P4 WebSocket 왕복 시간이 "측정불가"인 원인을 파악하는 데 가장 많은 시간이 소요됐다. k6 타임아웃이 서버 성능 문제인지 파이프라인 미연결 문제인지 구분할 수 없었다. Redis 채널에 Subscribe하는 리스너가 등록되지 않았다는 사실은 테스트 코드에 Mock이 존재했기 때문에 단위 테스트에서 드러나지 않았다.

`ChatSubscriber`가 존재하고 `MessageListener` 인터페이스를 구현하고 있었으나, `RedisMessageListenerContainer`에 등록된 적이 없었다. 컴파일과 단위 테스트는 통과하지만 실제 메시지 흐름은 연결되지 않은 상태였다.
