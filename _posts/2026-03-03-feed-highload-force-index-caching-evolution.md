...
결과적으로 FORCE INDEX 실패, cascading failure, 캐싱 전략 3번 교체를 거쳤다. 이 글은 "통과한 테스트 뒤에 숨어있던 문제들"에 대한 기록이다.
...
일반 테스트가 "전체 기능이 동작하는가"를 보는 것이라면, 고부하 테스트는 "어디서 터지는가"를 보는 테스트다.

| Phase | 목적 | VU | 공격 지점 |
|-------|------|-----|----------|
| P1 | 인기피드 CPU 정렬 | 400 | `LOG()+TIMESTAMPDIFF()` 매 row 연산 |
| P2 | 개인피드 IN절 폭발 | 400 | 유저당 20+ 클럽 → 대형 IN clause |
| P3 | 리포스트 + 상세 | 300 | JOIN FETCH + 댓글 전체 로드 |
| P4 | 좋아요 동시성 | 300 | Redis Lua + DB 스트림 컨슈머 경합 |
| P5 | 혼합 극한 | 400 | 읽기 + 쓰기 동시 |
| P6 | 딥 페이지네이션 | 200 | page 50~200 |

---

## 1차 고부하 결과 — 3대 병목 발견

| 순위 | 병목 | 심각도 | p95 | 슬로우 쿼리 | 원인 |
|------|------|--------|-----|-----------|------|
| 1 | 개인피드 IN절 | CRITICAL | 919ms | 36,094건 | `resolveAccessibleClubIds` 2회 호출 + 대형 IN clause |
| 2 | 인기피드 정렬 | HIGH | 567ms | 9,531건 | `LOG()+TIMESTAMPDIFF()` CPU-bound |
| 3 | 혼합 쓰기 | MEDIUM | 554ms | — | `comment_count++`과 `like_count++`이 같은 feed 행 X lock 경합 |
...
| 항목 | 수정 전 p95 | 수정 후 p95 | 개선 |
|------|-----------|-----------|------|
| 피드상세 | 970ms | 89ms | **-91%** |
| 댓글목록 | 614ms | 76ms | **-88%** |
| 클럽피드 | 440ms | 58ms | -87% |
| 혼합 읽기 | 886ms | 493ms | -44% |
| 혼합 쓰기 | 810ms | 554ms | -32% |

피드상세·댓글목록은 threshold를 통과했다. 하지만 P2 개인피드 IN절(p95 1,053ms)과 P5 쓰기(p95 554ms)는 여전히 위반이었다.

---

## FORCE INDEX 시도 — p95 29,930ms 대참사

개인피드 쿼리의 핵심 문제는 IN clause + ORDER BY + LIMIT 조합에서 옵티마이저가 filesort를 선택하는 것이다. 커버링 인덱스를 만들고 FORCE INDEX로 강제하면 해결될 것으로 판단했다.
...
**교훈: FORCE INDEX는 인덱스 존재 여부에 대한 런타임 의존성을 만든다. DDL 실행 순서와 결합하면 배포 시점에 장애가 발생할 수 있다.**
...
| | Redis pass1 | Redis 전체 (JSON) | 인메모리 |
|---|---|---|---|
| 캐시 히트 시 DB 쿼리 | 4~5개 | 0개 | 0개 |
| 직렬화 비용 | 경량 문자열 | ObjectMapper (무거움) | 없음 |
| 네트워크 I/O | Redis 왕복 | Redis 왕복 | 없음 |
| 스케일아웃 | 서버 간 공유 | 서버 간 공유 | 서버별 독립 |
| P2 p95 결과 | 986ms | 측정 불가 (CPU 병목) | **239ms** |
...
인메모리 캐싱으로 P1 읽기가 극적으로 개선된 뒤, 예상치 못한 현상이 발생했다.

| Phase | 항목 | 인메모리 적용 전 | 인메모리 적용 후 |
|-------|------|---------------|----------------|
| P1 | 개인피드 p95 | 2,742ms | **239ms** |
| P1 | 인기피드 p95 | 3,173ms | **215ms** |
| P2 | 좋아요 p95 | 260ms | **30,030ms** |
| P2 | 댓글작성 p95 | 263ms | **30,043ms** |
| P4 | 피드상세 p95 | 60,046ms | 31,483ms |
...
| 지표 | 1차 고부하 | 최종 | 총 개선 |
|------|----------|------|---------|
| P1 인기피드 p95 | 567ms | 238ms | -58% |
| P2 개인피드 p95 | 919ms | 239ms | **-74%** |
| P3 피드상세 p95 | 970ms | 89ms | -91% |
| P3 댓글목록 p95 | 614ms | 76ms | -88% |
| P5 혼합 읽기 p95 | 886ms | 495ms | -44% |
| P1 슬로우 쿼리 | 14,346건 | 69건 | **-99.5%** |
| 전체 처리량 | 353K iter | 529K iter | **+50%** |
| API 성공률 | 94.4% | 100% | |
...
고부하 테스트는 일반 테스트가 숨기는 문제를 드러낸다. 300VU에서 멀쩡하던 개인피드가 400VU + 유저당 20개 클럽이 되면 IN절이 폭발한다. 단건 API 응답이 189ms여도 동시 요청 300개가 되면 커넥션 풀이 고갈되어 30초 타임아웃을 친다.
