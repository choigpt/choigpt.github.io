...
- **2라운드**: 5개 도메인 코드 품질 일괄 정리 — 22개 이슈, 30개 파일 수정
...
분리 결과, common 모듈에 `GlobalErrorCode`(15개 상수 — SSE, DB, 글로벌), domain-user에 `UserErrorCode`(7개), domain-interest에 `InterestErrorCode`(2개), domain-club에 `ClubErrorCode`(9개), domain-chat에 `ChatErrorCode`(15개), domain-feed에 `FeedErrorCode`(6개), domain-finance에 `FinanceErrorCode`(22개), domain-schedule에 `ScheduleErrorCode`(14개), domain-notification에 `NotificationErrorCode`(6개), domain-image에 `ImageErrorCode`(5개), domain-search에 `SearchErrorCode`(8개)가 배치됐다.
...
공통 이슈는 네 가지다. **사일런트 예외 무시**(로깅 없이 삼킴)는 알림, 채팅, 피드에서 발견됐으며 심각도 High다. **`catch (Exception e)` 포괄적 예외 처리**는 알림, 피드, 금융에서 발견됐으며 심각도 Medium이다. **`var` 키워드 사용 불일치**는 알림, 금융에서 발견됐으며 심각도 Low다. **섹션 구분 주석 스타일 불일치**는 알림, 채팅, 피드에서 발견됐으며 심각도 Low다.
...
`NotificationRepositoryCustom.java`에서 `markSseSentByIds`를 `markDeliveredByIds`로 변경했고, `NotificationRepositoryImpl.java`의 구현 메서드와 `MysqlNotificationStorageAdapter.java`의 호출부도 동일하게 변경했다.
...
`ChatScheduleEventListener.java`에서 `IllegalStateException`을 `CustomException(ChatErrorCode.CHAT_ROOM_NOT_FOUND)`로 통일했다. `ChatPublisher.java`에서는 silent return에 `log.debug`를 추가했고, `MysqlChatMessageStorageAdapter.java`에서 `fromProjection`을 `toDto`로 변경하여 기존 매핑 메서드와 일관성을 맞췄다. `MongoChatMessageStorageAdapter.java`에서는 `// ========== x ==========` 구분자를 `// ── x ──`로 통일했다.
...
`Settlement.java`에서 TODO와 주석 import, 주석 코드를 삭제했다. `SettlementRepository.java`에서 TODO와 주석 import를 삭제했다. `UserSettlementRepository.java`에서는 **약 60줄의 레거시 주석 코드 전체**를 삭제했다. `Transfer.java`에서도 TODO와 주석 import, 주석 코드를 삭제했다.

`UserSettlementRepository`에 주석처리된 쿼리가 60줄 가까이 남아 있었다. 순환 의존성 방지를 위해 임시로 주석처리한 것인데, ErrorCode 분리로 순환이 해소된 뒤에도 그대로 남아 있었다.

**매직넘버 상수화**

`SettlementKafkaEventListener.java`에서 매직넘버 `10000`을 `OUTBOX_AGGREGATE_MULTIPLIER`로, `10`(seconds)을 `FUTURE_TIMEOUT_SECONDS`로 상수화했다. `RedisLuaService.java`에서 `5`를 `MAX_RETRY_ATTEMPTS`로, `5`와 `20`을 `RETRY_MIN_DELAY_MS`와 `RETRY_MAX_DELAY_MS`로 상수화했다. `UserSettlementService.java`에서 `10`을 `WALLET_GATE_TTL_SECONDS`로 상수화했다.
...
`ClubSearchResult.java`에서 `interestKoreanName`을 `interest`로, `clubImage`를 `image`로 변경했다. `ElasticsearchSearchAdapter.java`와 `MysqlFulltextSearchAdapter.java`에서 매핑 필드명을 갱신했고, `SearchService.java`의 변환부 필드명도 함께 갱신했다.
...
수정 총괄: **Notification**(5개 파일) — 용어 정규화, 예외 로깅, var → 타입. **Chat**(4개 파일) — 예외 타입 통일, 네이밍 일관성, 구분자 통일. **Feed**(6개 파일) — 중복 record 삭제, 캐시 키 상수화, 예외 로깅. **Finance**(11개 파일) — 레거시 70줄 삭제, 매직넘버 상수화, @Slf4j 추가. **Search**(4개 파일) — 필드명 통일, enum 활용, 중복 JOIN 제거. 합계 **30개 파일**.
...
**코드 품질 정리는 성능 테스트 직후가 적기다.** 성능 테스트에서 코드를 많이 건드렸으니 어디가 지저분한지 기억이 선명하다. 시간이 지나면 `UserSettlementRepository`의 주석 60줄이 왜 존재하는지, `settlementId * 10000`이 무슨 의미인지 파악하기 어렵다.
