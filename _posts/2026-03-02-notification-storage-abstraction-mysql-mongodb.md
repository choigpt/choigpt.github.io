...
[SSE 부하 테스트 글](/sse-notification-load-test-analysis/)에서 SSE 시나리오별 부하 테스트를 마쳤다. 개별 도메인 최적화가 끝난 시점에서 다음 과제는 Storage와 Delivery 계층을 교체 가능하게 만드는 것이었다. 채팅 도메인에서 `ChatMessageStoragePort` 추상화를 먼저 진행한 경험이 있었고, 알림 도메인에도 동일한 Port/Adapter 패턴을 적용했다.
...
`NotificationStoragePort`는 8개 메서드로 알림 CRUD, 읽음 처리, 미읽음 카운트, 미전송 조회를 담당한다. `NotificationDeliveryPort`는 3개 메서드로 실시간 전달(connect, send, disconnect)을 처리한다. `NotificationEventPublisher`는 1개 메서드로 알림 생성 이벤트를 발행한다.
...
변경 규모는 신규 파일 6개(Port 3 + Adapter 3)와 수정 파일 10개(Service, Controller, Config 등)다.
...
판정 **16/16 PASS**, Hot User p95 1.59초.
...
판정 **8/16 FAIL**. 처리량은 1,090에서 79 req/s로 **13.8배 하락**, Write Storm p95는 508ms에서 10,190ms로 **20배 악화**됐다.

Stage 4(1.41M)에서 Stage 5(14M)로 넘어오며, Write Storm p95는 508ms에서 10,190ms로 20배, Hot User p95는 1,590ms에서 5,820ms로 3.7배 악화됐다. 처리량은 1,090 req/s에서 79 req/s로 13.8배 하락했고, 실패율은 0%에서 12.73%로 올라갔다.
...
```mermaid
xychart-beta
    title "14M 데이터: MySQL vs MongoDB (p95, ms)"
    x-axis ["Write Storm", "Hot User"]
    y-axis "ms" 0 --> 11000
    bar [10190, 5820]
    bar [9.61, 12.08]
```

MySQL은 8/16 FAIL인 반면 MongoDB는 **16/16 ALL PASS**. Write Storm p95는 MySQL 10,190ms, MongoDB **9.61ms**로 **1,061배** 차이. Hot User p95는 MySQL 5,820ms, MongoDB **12.08ms**로 **482배** 차이. 처리량은 MySQL 79 req/s, MongoDB **1,686 req/s**로 **21배** 차이. 실패율은 MySQL 12.73%, MongoDB **0.03%**.
...
MySQL은 `UPDATE ... WHERE user_id=? AND is_read=false`로 14,000+ 행에 개별 row lock을 걸어 같은 유저의 다른 쿼리가 전부 대기한다. MongoDB는 `updateMulti(query, update)`로 document-level lock을 단일 명령으로 처리하며, 다른 document 접근에 영향을 주지 않는다.
...
> 트레이드오프: MongoDB의 `updateMulti`는 document 단위로 lock을 잡고 놓으므로, 전체 갱신이 **atomic하지 않다**. 갱신 중간에 다른 읽기 요청이 들어오면 일부 알림은 읽음, 일부는 미읽음인 부분 갱신 상태를 볼 수 있다. `markAllAsRead` 같은 사용자 액션에서는 이 정도의 일시적 불일치가 UX에 영향을 미치지 않으므로 수용했다.

### MongoDB 스케일 테스트: 600VU

MongoDB가 14M에서 안정적이었으므로 VU를 추가로 올려 테스트했다.

500VU에서 16/16 PASS, 처리량 1,686 req/s. 600VU에서도 **16/16 PASS**, 처리량 **2,471 req/s**.
