...
| 경합 회피 | 대기 or deadlock (SKIP LOCKED 지원하나 UPDATE문에 미적용) | `FOR UPDATE SKIP LOCKED` + MVCC |
...
MySQL은 `UPDATE ... WHERE user_id = ? AND is_read = false`에서 gap lock이 걸려 동일 사용자의 다른 알림 INSERT까지 블로킹된다. MySQL 8.0+에서 `SKIP LOCKED`를 지원하지만 이는 `SELECT ... FOR UPDATE`에만 적용 가능하고, `UPDATE ... WHERE` 구문에는 직접 사용할 수 없다. PostgreSQL은 MVCC 기반으로 UPDATE 시 row-level lock을 짧게 잡아 동일 시나리오에서 경합이 크게 완화된다. 이 시나리오가 두 DB의 가장 극명한 차이를 보여주는 케이스였다.
...
batch write 경합에서 명확한 우위를 보인다. `FOR UPDATE SKIP LOCKED`는 MySQL 8.0+에서도 지원하지만, 알림의 markAllAsRead는 `SELECT ... FOR UPDATE`가 아닌 `UPDATE ... WHERE` 구문이라 SKIP LOCKED를 직접 적용할 수 없었다. PostgreSQL은 MVCC 기반으로 이러한 batch UPDATE 경합 자체가 완화되어 정산과 알림의 multi-row 갱신 시나리오에서 deadlock을 원천 차단한다. 정산/알림 트래픽이 현재의 10배 이상으로 증가하면 마이그레이션을 검토할 가치가 있다.
