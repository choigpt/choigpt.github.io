...
|--------|---------|----------|----------|
| 알림 | 1,500 (MySQL) / 6,000 (MongoDB 비교) | MySQL write 붕괴 (mark p50 29s), SSE 전달률 63.2% | MongoDB 비교: mark p50 30ms, 커널/JVM 튜닝 후 iterations +20% |
...
| 정산 | 1,500 | COMPLETED 0건, row lock 10,832건 | p95 18.64s → 1.50s, 5xx 0% (Round 5 기준, 확인 테스트 Soak 5xx 7.20%) |
...
| 설정 | 현재 | 권장 | 비고 |
|------|------|------|------|
| innodb_buffer_pool_size | 3GB | 8~10GB | 데이터+인덱스 32GB 대비 9.4%. 권장 70~80% |
...
| 인프라 튜닝 | 커널, Buffer Pool, redo log | 커널: 알림·채팅에서 적용 완료, redo log: 피드에서 적용, Buffer Pool: 미적용 |
