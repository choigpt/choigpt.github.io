...
단, 이 배수는 MySQL이 6,000 VU에서 완전히 붕괴한 상태(mark p50 29초, delete p50 59초)와 정상 작동 중인 MongoDB를 비교한 수치다. 둘 다 정상 작동하는 1,500 VU 구간에서 비교하면 MySQL mark p95 1,210ms vs MongoDB ~30ms로, 실질적 차이는 약 40배다. 40배도 충분히 유의미한 차이이며, 핵심은 배수가 아니라 MySQL이 특정 부하에서 구조적으로 붕괴한다는 사실이다.
