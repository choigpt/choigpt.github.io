...
이전 글: [EC2 다운사이징 후 최적화 (상) — 피드·채팅·알림](/ec2-downsizing-optimization-part1/)
...
| Stuck 복구 스케줄러 | 5분마다 IN_PROGRESS 5분 초과 → FAILED | 정산 hang 방지 |
...
| 지표 | 유휴 | 부하 중 |
|------|------|---------|
| threads_running | 3 | 349 |
| slow_queries | +0/분 | +100/분 |
| HikariCP pending | 0 | 238 |
...
| 용도 | 키 패턴 | TTL |
|------|---------|-----|
| 결제 멱등성 게이트 | `payment:gate:{orderId}` | 5분 |
| 결제 정보 임시 저장 | `payment:{orderId}` | 30분 |
| 지갑 분산 락 | `wallet:gate:{userId}:{op}` | 10초 |
...
다음 글: [EC2 다운사이징 후 최적화 (하) — 전체 통합 결과와 코드 리뷰](/ec2-downsizing-optimization-part3/)
