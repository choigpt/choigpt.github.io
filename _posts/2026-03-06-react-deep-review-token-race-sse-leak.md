...
AI 리뷰는 놓치기 쉬운 포인트를 빠르게 찾아주는 데 도움이 됐지만, 결국 교차검증은 직접 해야 한다. 어디까지나 참고용이고, 지적 사항을 이해하고 판단하려면 해당 영역을 공부하는 과정이 반드시 필요했다.
...
```mermaid
xychart-beta
    title "심화 리뷰 영역별 점수 (10점 만점)"
    x-axis ["API레이어", "라우터", "Auth", "컴포넌트", "타입안전성", "SSE", "채팅소켓", "테스트", "빌드설정"]
    y-axis "점수" 0 --> 10
    bar [6.5, 8.5, 6, 7, 6.5, 6, 7.5, 4, 8.5]
```
...
1. **client.test.ts** (13개) — 요청/응답 인터셉터, 토큰 리프레시 mutex, HTTP 메소드 5종
2. **upload.test.ts** (12개) — 파일 타입 검증, presigned URL, S3 업로드
3. **useDynamicTitle.test.ts** (8개) — 채팅방/모임 제목 로딩, API 실패 폴백, abort cleanup
4. **Toast.test.tsx** (8개) — ToastContext, showToast, 자동닫기, 타입별 스타일
5. **ErrorBoundary.test.tsx** (6개) — 에러 catch, 기본/커스텀 fallback, 로깅
6. **KakaoCallback.test.tsx** (7개) — OAuth 콜백 전체 플로우 (성공/실패/신규사용자/403)
7. **kakaoAuth.test.ts** (5개) — Kakao SDK 초기화, 로그인 리다이렉트
8. **getNotificationMessage.test.ts** (5개) — 알림 메시지 생성 (settlement, comment, like, chat)
