...
| 대상 도메인 | 피드, 채팅, 알림, 정산, 검색, 클럽-일정 (6개) |
...
| 9000 VU (3x 시드) | 안정 | 안정, 전 엔드포인트에서 더 빠름 |

**채택: MongoDB** — mark-all의 O(N) UPDATE가 MySQL의 구조적 한계. MongoDB document-level lock이 유리.
...
**채택: ES** — 키워드 검색 830배 차이. MySQL FULLTEXT는 ES 불가 시 폴백용.
