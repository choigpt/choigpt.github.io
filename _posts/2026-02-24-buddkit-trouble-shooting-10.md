...
1화와 2화에서 인증 인프라의 버그들을 정리했다. SSE 필터가 subject를 kakaoId로 착각해 SSE 연결이 전면 불통이었던 것, `JwtException` 패키지 불일치로 만료 토큰에서 500이 터졌던 것, 로그아웃이 실제로 카카오 연결을 해제하고 있었던 것. 그 버그들은 인증 흐름 자체의 문제였다.
...
`getCurrentUserId()`가 kakaoId를 반환한다는 사실은 1화에서 `SearchService` 예시로, 5화에서 모임 검색의 `isJoined=false` 문제로 각각 부분적으로 언급됐다. 두 화에서는 해당 서비스의 증상만 고쳤고, 근본 원인인 메서드 자체는 이번에 수정한다. 인증 이후 모든 비즈니스 로직의 출발점이 되는 메서드가 잘못된 값을 흘려보내고 있었다.
...
인증은 필터에서 처리되지만, 생명주기의 각 전환점은 UserService에서 처리된다. 필터가 올바르게 동작해도 각 전환점에 검증이 없으면 비정상 상태로 서비스에 진입할 수 있었다.
...
1화에서 `SearchService`의 `findByClubIdsByUserId(userId)`에 kakaoId가 넘어가는 예시를 다뤘고, 5화에서는 모임 검색의 `isJoined=false` 문제로 다시 언급됐다. 두 화에서는 해당 서비스의 증상만 고쳤고, 근본 원인인 `getCurrentUserId()` 자체는 이번에 수정한다. 이 메서드를 호출하는 곳은 14개였다.

```
SseStreamController, NotificationController, WalletService, PaymentService,
ScheduleService, SettlementService, ClubService, FeedService, FeedMainService,
ChatRoomService, MessageRestController, SearchService, AuthController
```
...
`SecurityContext`에 `UserPrincipal`을 직접 담도록 바꾸고, `getUserId()`로 userId(DB PK)를 바로 꺼낸다. DB 조회도 사라졌다.

---

### INACTIVE 체크 — `String.equals(Enum)`은 항상 false

2화에서 `@Profile("!test")`를 제거해 필터가 테스트 환경에서도 실행되게 됐다. 그런데 필터를 실제로 돌려보니 코드 자체가 잘못돼 있었다.

```java
// JwtAuthenticationFilter.java:67 — 수정 전
...
탈퇴한 사용자의 userId로 알림 전송이 시도되고, 스케줄 서비스가 해당 사용자를 참여자로 포함해 로직을 실행하고, 정산 서비스가 INACTIVE 사용자의 지갑에 접근하는 상황이 조용히 지속될 수 있었다.
...
MySQL에서 `null + amount = null`, `100000 - null = null`이다. 신규 사용자가 첫 스케줄에 참여할 때 hold 연산이 항상 실패했다. 9화에서 정산 도메인 작업 중 함께 발견해 `@Builder.Default 0L`로 초기화를 적용했다.
...
Refresh Token은 2화에서 사실상 동작하지 않는 상태로 언급됐다. 생성은 되지만 재발급 엔드포인트가 없어서다. `POST /api/v1/auth/refresh` 엔드포인트를 구현하고 화이트리스트에 등록했다.
...
| Wallet pendingOut null → hold 연산 실패 | ✅ 해결 — @Builder.Default 0L (9화에서 적용) |
...
인증 파이프라인의 버그들(1~2화)은 눈에 띄었다. SSE가 전면 불통이었고, 만료 토큰이 500을 반환했다. 분명한 증상이 있었다.
...
이런 버그들은 정상 경로에서는 잘 동작하다가 경계 조건에서 터진다. 탈퇴 직후, 회원가입 미완료 상태에서, 의도적으로 API를 중복 호출할 때. 사용자 생명주기의 각 전환점에 검증이 없으면 정상 플로우에서는 멀쩡해 보이다가 경계 상황에서 예상치 못한 방식으로 실패한다.
