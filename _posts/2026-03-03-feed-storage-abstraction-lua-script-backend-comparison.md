...
[이전 글](/feed-highload-force-index-caching-evolution/)에서 피드 도메인의 고부하 테스트를 통과시켰다. 9라운드 쿼리 최적화, FORCE INDEX 실패, 인메모리 캐싱 진화까지 거친 뒤 남은 과제는 두 가지였다.

1. Storage를 교체 가능하게 만들어서 MySQL 외 대안을 실측할 것
2. 좋아요 토글의 Lua Script 구조가 고부하에서도 버티는지 검증할 것

채팅 도메인에서 이미 [ChatMessageStoragePort 추상화](/chat-storage-abstraction-websocket-extreme/)를 진행한 경험이 있었다. 같은 Port/Adapter 패턴을 피드 도메인에 적용하되, 피드 특유의 복잡성(이미지, 클럽, 좋아요 카운트, 리포스트)을 반영한 설계가 필요했다.
...
| Record | 역할 |
|--------|------|
| `FeedProjection` | 조회 결과 — feedId, content, user, club, images, counts |
| `FeedSaveDto` | 생성 입력 — content, clubId, userId, imageUrls |
| `FeedUpdateDto` | 수정 입력 — content, imageUrls |

### 어댑터 구현

| 파일 | 역할 |
|------|------|
| `port/FeedStoragePort.java` | 저장소 추상화 인터페이스 |
| `port/MysqlFeedStorageAdapter.java` | MySQL 어댑터, `matchIfMissing=true` 기본값 |
| `port/MongoFeedStorageAdapter.java` | MongoDB 어댑터 |
| `entity/FeedDocument.java` | MongoDB document |
| `config/MongoFeedConfig.java` | MongoDB 설정 |
...
| 클래스 | 변경 내용 |
|--------|----------|
| `FeedCacheService` | `FeedRepository` 직접 참조 제거, `FeedStoragePort` 주입 |
| `FeedQueryService` | 조회 로직 전체를 Port 경유로 변경 |
| `FeedRenderService` | DTO 변환 로직에서 Port의 `FeedProjection` 사용 |
...
| 순서 | 명령 | 역할 |
|------|------|------|
| 1 | `SISMEMBER` | likers SET에서 유저 존재 여부 확인 |
| 2 | `SADD` / `SREM` | likers SET에 추가 또는 제거 |
| 3 | `INCRBY` | like_count 증감 |
| 4 | `XADD` | like:events Stream에 이벤트 발행 |
...
|------|--------|--------|------|
| p95 | 951ms | 514ms | -46% |
| Throughput | 972 req/s | 1,770 req/s | +82% |
| Success Rate | 99.94% | 99.96% | - |
...
| 구성 | p95 (1000VU) | Throughput | Success Rate |
|------|-------------|------------|-------------|
| MySQL existsById (original) | 951ms | 972 req/s | 99.94% |
| MySQL + Redis cache | 514ms | 1,770 req/s | 99.96% |
| MongoDB + Redis cache | 467ms | 1,769 req/s | 99.97% |
...
| 시나리오 | MySQL + Redis cache | MongoDB + Redis cache | 차이 |
|----------|--------------------|-----------------------|------|
| Mixed read+write p95 | 401ms | 318ms | -21% |
...
| 항목 | 결과 |
|------|------|
| Storage 추상화 | FeedStoragePort 11개 메서드 + 3개 DTO, MySQL/MongoDB 어댑터 |
| Lua Script | 수정 불필요 — 1,000 VUs conflict 0 |
| 진짜 병목 | existsById MySQL 쿼리 — Redis 캐싱으로 throughput +82% |
| MySQL vs MongoDB | throughput 동일, MongoDB는 mixed read+write p95에서 -21% |
| MongoDB 한계 | 개인/인기 피드 aggregation pipeline 호환 실패 |
| 최종 선택 | MySQL primary 유지, Port/Adapter 추상화 보존 |
