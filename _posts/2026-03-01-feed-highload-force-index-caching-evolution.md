...
피드상세·댓글목록은 threshold를 통과했다. 하지만 P2 개인피드는 919ms → **1,053ms로 오히려 악화**했고, P5 쓰기(p95 554ms)도 여전히 위반이었다. 1차 수정의 인덱스 추가가 다른 쿼리를 살리는 대신 IN절 옵티마이저 선택을 더 나쁜 쪽으로 흔든 것으로 보였고, 이게 다음 절의 FORCE INDEX 시도로 이어진다.
...
> 주의: 이 p95는 정상 응답시간 분포가 아니다. 인덱스가 없어 500 에러를 반환하다가 클라이언트가 30초 타임아웃까지 기다린 케이스가 누적된 결과로, 사실상 "에러 시나리오의 응답시간"이다. 같은 환경에서 정상 응답으로 측정한 p95가 아니라는 점을 염두에 두고 읽어야 한다.
...
2. FORCE INDEX는 **인덱스가 존재하지 않으면 에러를 반환**한다(`ER_KEY_DOES_NOT_EXIST` 1176). Spring이 이를 500 에러로 변환했고, 400VU가 동시에 500을 받으면서 커넥션 풀이 에러 처리로 포화됐다.
3. 에러 응답을 받은 클라이언트가 다시 시도하거나 응답을 기다리면서 k6의 trend metric에 타임아웃까지의 대기 시간이 그대로 누적됐고, 이로 인해 p95가 30초까지 치솟았다.
