...
| P3 | 혼합 (조회/전송/삭제) | 200 |
| P4 | WebSocket | 150 |
| P5 | 스파이크 (단일 채팅방 동시 접속) | 500 |

> 1차에서는 P5 스파이크를 800 VU로 잡았지만 단일 호스트 한계로 중간에 400 VU로 조정했고, 3차 최종 측정은 위 표 기준으로 다시 돌렸다. 본문에 "800VU"·"400VU"가 함께 등장하는 것은 그 흐름의 흔적이다.

### 3M+ 메시지, 위 Phase 구성 기준 최종 수치
