...
| 컴포넌트 | 역할 | 예시 |
|----------|------|------|
| **ItemReader** | 데이터 소스에서 데이터를 읽어온다 | DB 쿼리, CSV 파일 읽기, API 호출 |
| **ItemProcessor** | 읽어온 데이터를 가공/변환/필터링한다 | 유효성 검증, 포맷 변환, 비즈니스 규칙 적용 |
| **ItemWriter** | 가공된 데이터를 저장한다 | DB 저장, 파일 출력, 외부 시스템 전송 |
