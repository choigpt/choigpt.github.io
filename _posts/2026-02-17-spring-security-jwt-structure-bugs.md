...
Security 관련 코드를 점검한 결과, 버그와 설계 문제가 의존성, 필터, OAuth 흐름, 비즈니스 로직 등 여러 영역에 걸쳐 존재했다.

그중 컴파일과 테스트를 모두 통과하지만 런타임에서 **만료된 토큰이 401이 아닌 500을 반환하는 버그**가 있었다. 원인은 `import` 패키지 불일치였다.

이 외에도 로그아웃이 실제로 카카오 연결을 해제(`unlink`)하고 있었고, `SecretKey` 생성 시 charset이 세 곳에서 제각각이었으며, `SecurityConfig`에 `@Profile("!test")`가 붙어 있어 보안 필터가 테스트 환경에서 아예 로드되지 않았다.
...
Refresh Token은 생성해서 클라이언트에 반환하지만 재발급 엔드포인트가 없어 사실상 동작하지 않는 상태였다. 재발급 구현은 [유저 도메인 편](/user-lifecycle-bugs/)에서 처리했다. `/auth/me` 응답에 내부 식별자인 `kakaoId`가 그대로 노출되고 있었다.

`JwtAuthenticationFilter`의 INACTIVE 체크 코드(`Status.INACTIVE.name().equals(user.getStatus())`)는 `String.equals(Enum)` 구조라 항상 false를 반환하는 문제도 있었다. 이 수정은 `UserPrincipal` 구조 개편과 묶어 유저 도메인([유저 도메인 편](/user-lifecycle-bugs/))에서 함께 처리했다.
...
`@Profile("!test")`를 제거해 `SecurityConfig`가 항상 로드되도록 변경했다. 테스트 환경에서는 `IntegrationTestConfig`의 `@Order(0)` `SecurityFilterChain`이 우선 적용되어 전체 `permitAll`을 유지한다. `IntegrationTestConfig`는 이번에 새로 작성했다.
...
> 참고: 이 시점에서는 `SseAuthenticationFilter`와 `JwtAuthenticationFilter` 두 필터 구조가 유지된다. `/sse/**` 경로는 `SseAuthenticationFilter`가 전담하고, `JwtAuthenticationFilter`에 `shouldNotFilter`를 추가해 이중 실행을 방지했다. 두 필터 구조의 복잡성 문제는 [필터 단일화 편](/sse-auth-filter-removal/)에서 `SseAuthenticationFilter`를 완전히 제거하고 `JwtAuthenticationFilter`로 통합하면서 해소된다.
...
| 문제 | 상태 |
|------|------|
| `JwtException` 패키지 불일치 → 만료 토큰 500 | ✅ 해결 — `io.jsonwebtoken.JwtException`으로 수정 |
| jjwt 이중 선언 + 스타일 혼재 | ✅ 해결 — 0.12.4 단일 버전, 0.12.x 스타일로 통일 |
...
| `withdraw` 이중 호출 가능 구조 | ✅ 해결 — [유저 도메인 편](/user-lifecycle-bugs/)에서 `try-catch` 정리, 단일 호출로 변경 |
| Refresh Token 재발급 엔드포인트 없음 | ✅ 해결 — [유저 도메인 편](/user-lifecycle-bugs/)에서 구현 |
| `/auth/me` kakaoId 노출 | ✅ 해결 — `UserInfoResponse`에서 제거 |
| INACTIVE 체크 `String.equals(Enum)` | ✅ 해결 — [유저 도메인 편](/user-lifecycle-bugs/)에서 `UserPrincipal.isEnabled()` Enum 비교로 변경 |

---

## 정리하며

보안 관련 코드는 컴파일과 테스트를 통과하더라도 런타임에서 다른 동작을 할 수 있다. `JwtException` 패키지 불일치가 대표적인 사례로, 컴파일 에러 없이 만료 토큰에서 500이 발생하는 버그가 존재했다.
...
> 같은 이름의 클래스가 여러 패키지에 존재할 때 IDE 자동완성에 의존하지 않아야 한다. 특히 예외 처리처럼 흐름을 제어하는 코드에서는 import 경로까지 확인해야 한다.
