...
패치 후 구조를 재검토한 결과, 구조 자체가 불필요하게 복잡했다. SSE 전용 필터와 일반 요청 필터, 두 개의 JWT 필터가 서로 경로를 나눠 갖고 있었다. `SseAuthenticationFilter`가 존재해야 하는 이유는 브라우저의 `EventSource` API가 커스텀 헤더를 지원하지 않아 SSE 연결 시 쿠키로 토큰을 전달해야 한다는 것이었다. 하지만 이 쿠키 읽기 로직은 `SseAuthenticationFilter`만의 것이 아니라 `JwtAuthenticationFilter`에도 들어갈 수 있었다.
