...
알림 API와 SSE 실시간 전송 코드를 점검한 결과 13개의 이슈가 확인됐다. 단독 버그 외에, 세 개의 버그가 결합되어 **오프라인 사용자의 알림이 전송된 것으로 기록되고, 재접속 후에도 복구되지 않는** 시나리오가 발생할 수 있었다. 개별적으로는 경미한 문제지만, 조합 시 알림이 누락되는 결과를 초래한다.
