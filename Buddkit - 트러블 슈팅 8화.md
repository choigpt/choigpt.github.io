---
title: 지갑·결제 도메인 — 토스 승인 후 DB가 죽으면 돈이 사라진다
date: 2026-02-20
tags: [결제, 동시성, JPA, 트러블슈팅, Java, 지갑, 토스페이먼츠]
---

# 지갑·결제 도메인 — 토스 승인 후 DB가 죽으면 돈이 사라진다
...
클라이언트 → POST /payments/save
    클라이언트가 전달한 amount를 Redis에 저장
    ↓
클라이언트 → POST /payments/success
    Redis에서 amount 조회 → 클라이언트 전달 amount와 비교
    ↓
클라이언트 → POST /payments/confirm
    토스페이먼츠 결제 승인 API 호출 (외부)
    ↓
    지갑 비관적 락 획득
    ↓
    wallet.updateBalance(wallet.getPostedBalance() + amount)  ← 문제
    ↓
...
`/payments/save`에서 클라이언트가 전달한 `amount`를 그대로 Redis에 저장하고, `/payments/success`에서 다시 클라이언트가 전달한 값과 비교한다. 서버가 "이 주문은 얼마여야 한다"를 독립적으로 결정하는 로직이 없다. 비교의 양쪽이 모두 클라이언트 입력이라 의미 있는 검증이 아니다.

실제 결제는 토스 SDK에서 사용자가 결제한 금액으로 이루어지므로 `/confirm`의 amount가 실제 결제액과 다르면 토스 API가 거부한다. 하지만 `/save` 단계에서 서버가 독립적으로 금액을 생성하고 저장해야 한다는 원칙이 지켜지지 않고 있었다.

---

### DB 락 잡은 채 외부 API 호출 — 데드락 위험

```java
// PaymentService.java
Wallet wallet = walletRepository.findByUser(user);  // FOR UPDATE → 행 락 획득
// ...
response = tossPaymentClient.confirmPayment(req);   // 외부 API (수 초 소요)
// 락을 잡은 채로 토스 응답을 기다림
```

`findByUser()`에 `@Lock(PESSIMISTIC_WRITE)`가 걸려 있어 Wallet 행에 배타 락이 잡힌다. 그 상태에서 토스페이먼츠 API를 호출하면 응답이 오는 동안 락이 유지된다. 이 사이에 정산 프로세스가 같은 행을 수정하려 하면 대기 상태가 된다. 토스 응답이 느릴수록 대기 시간이 길어지고, 커넥션 풀이 고갈되면 다른 요청들까지 연쇄적으로 밀린다.
...
`long`은 null이 될 수 없으므로 `@NotNull`은 의미가 없다. JSON에서 `amount` 필드가 누락되면 기본값 0이 들어온다. 0원 결제가 허용된다. `Long` wrapper 타입으로 바꾸거나 `@Min(1)`을 추가해야 한다.

**`Wallet.pendingOut` 초기값 누락**

`UserService`에서 지갑 생성 시 `postedBalance`는 초기값을 명시적으로 설정하지만 `pendingOut`은 설정하지 않는다. DB 레벨 default도 없으면 `null`이 들어가고, 이후 `pending_out + :amount` native query에서 `null` 연산 오류가 발생한다.
...
완벽한 분산 트랜잭션은 아니지만, 취소 API 호출 자체가 실패하는 경우를 대비해 결제 상태를 별도로 기록해두고 추후 수동 처리할 수 있는 경로도 함께 마련했다.
...
`confirm()`의 `wallet.updateBalance(wallet.getPostedBalance() + amount)` 호출을 제거하고 `creditByUserId()`로 교체했다. 정산·홀드 경로와 동일한 방식으로 통일됐다. DB가 현재 값을 읽고 더하는 단일 연산이므로 동시 실행에서도 값이 덮어써지지 않는다. 비관적 락도 더 이상 필요 없어졌다.

---

### DB 락 분리 — 락 해제 후 외부 API 호출

지갑 잔액 업데이트를 `creditByUserId` native query로 바꾸면서 `findByUser(PESSIMISTIC_WRITE)`가 불필요해졌다. 토스 API 호출 시점에 DB 행 락이 잡혀 있지 않으므로 외부 API 응답을 기다리는 동안 다른 트랜잭션이 블로킹되지 않는다.
...
| DB 락 잡은 채 외부 API 호출 | ✅ 해결 — native query 전환으로 락 불필요 |
| 결제 금액 서버 검증 없음 | ✅ 해결 — 서버 측 금액 생성·저장으로 변경 |
...
외부 결제 API가 끼면 복잡도가 한 단계 더 올라간다. DB 트랜잭션으로 롤백할 수 있는 범위와 외부 API 호출로 이미 처리된 범위가 다르기 때문이다. 토스가 승인하고 나면 DB 롤백으로는 되돌릴 수 없다. 보상 트랜잭션을 명시적으로 설계하지 않으면 그 틈새에서 돈이 사라진다.

> **외부 시스템과의 경계에서는 롤백이 보장되지 않는다.**
> DB 트랜잭션이 실패하면 롤백되지만, 외부 API가 이미 처리한 것은 롤백되지 않는다.
> 그 틈새를 메우는 보상 경로를 반드시 설계해야 한다.
